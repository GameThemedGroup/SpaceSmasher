...
public class UserCode extends SpaceSmasher
{
  ...
  /** 
   * Ensure the blockset is cleared when moving to the next level, or newly added
   * blocks will spawn in incorrectly. Clearing the blockSet will clear any blocks
   * still left. Setting the ball invisible will take it out of play. 
   */
  ...
  /** 
   * Creates a level that is populated with random blocks. Be sure to clear the blockSet
   *  first, by calling blockSet.clear().
   *
   * @param columns
   *          - The number of columns to create in the level
   * @param rows
   *          - The number of rows to create in the level
   */
  protected void loadLevel(int columns, int rows)
  {
	  blockSet.setBlocksPerRow(columns);
	  for(int loop = 0; loop < blockSet.getNumColumns() * rows; loop++)
	  {
  ...
  protected void update()
  {
    ...
    // If there are no blocks left on the screen..
    if(blockSet.allBlocksAreDead())
    {
    	...
    	else // Otherwise, there are more levels..
    	{
    		nextLevel(); 	// Increment the level, update the level text
    		this.clearLevel(); 	// Clear the block set and reset the ball
    		switch(getLevel()) // Find which level we've incremented to..
    		{
    			case 1: // Level 1.. 
    				this.loadLevel(3, 5); // Create a 3 column, 5 row level
    				break;
    			case 2:
    				this.loadLevel(7, 5); // .. 7 column, 5 row level
    				break;
    			default:
    				this.loadLevel(2, 5); // .. 2 column, 5 row level
    				break;
    		}
    	}
    }
